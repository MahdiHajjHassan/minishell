...
static char *create_heredoc_temp(char *delimiter, int expand_vars)
{
    ...
    ft_strlcpy(temp_filename, "/tmp/minishell_heredoc_parse", sizeof(temp_filename));
    temp_fd = open(temp_filename, O_CREAT | O_EXCL | O_WRONLY, 0600);
    if (temp_fd < 0) {
        // If file exists, try with different suffix
        ft_strlcpy(temp_filename, "/tmp/minishell_heredoc_parse_tmp", sizeof(temp_filename));
...
static struct s_cmd *handle_heredoc_redir(struct s_cmd *cmd, char *file)
{
    ...
    char *temp_filename = create_heredoc_temp(delimiter, expand_vars);
    free(delimiter);
    ...
    return redircmd(cmd, temp_filename, temp_filename + ft_strlen(temp_filename), O_RDONLY, 0);
}
